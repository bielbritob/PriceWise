...
async def search_ig():
    ...
    print("selected 'AV. SETE DE SEPTEMBER' my friend...")
    ...
    print(f"produto  IG achado?: {isFound}")
    if isFound != None:
        print(f"NENHUM PRODUTO ACHADO! '{produt}'" )
        dados_produtos.append("NENHUM PRODUTO ACHADO!")
    else:
        FourZeroThreeError = await page.query_selector("h1")
        print(FourZeroThreeError)
        if FourZeroThreeError != None:
            print('VC FOI DETECTADO!!')
            await page.save_screenshot(filename=f"ig.com.br/{produt}2.png", full_page=True)
        # ----------------garanty#
        #await page.wait_for('iv.relative.px-4.shadow-md.border.rounded-lg.min-h-max.pb-2.h-full', timeout=5)
                                                #relative px-4 shadow-md border rounded-lg min-h-max pb-2 h-full
